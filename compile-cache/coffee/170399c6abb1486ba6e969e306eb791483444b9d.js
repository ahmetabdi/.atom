(function() {
  ...
  describe("Whitespace", function() {
    var buffer, editor, _ref;
    _ref = [], editor = _ref[0], buffer = _ref[1];
    beforeEach(function() {
    ...
    describe("when 'whitespace.ignoreWhitespaceOnlyLines' is true", function() {
      ...
      return it("removes the wthiespace from all lines, excluding the whitespace-only lines", function() {
        editor.insertText("1  \n2\t  \n\t \n3\n");
        editor.moveCursorToBottom();
        editor.save();
        return expect(editor.getText()).toBe("1\n2\n\t \n3\n");
    ...
    describe("GFM whitespace trimming", function() {
      beforeEach(function() {
        ...
        return runs(function() {
          return editor.setGrammar(atom.syntax.grammarForScopeName("source.gfm"));
        });
      });
    ...
    return describe("when the 'whitespace:convert-spaces-to-tabs' command is run", function() {
      return it("removes all space characters and replaces them with hard tabs", function() {
        ...
        expect(buffer.getText()).toBe('\ta\n\t\nb\t\nc\t\td');
        editor.setTabLength(3);
        return buffer.setText("   a\n   \nb   \nc      d", atom.workspaceView.trigger('whitespace:convert-spaces-to-tabs'), expect(buffer.getText()).toBe('\ta\n\t\nb\t\nc\t\td'));
      });
    });
  });
